...
const availableVariant = computed(() => product.value?.variants?.find((variant) => variant.stock > 0) || null);
...
<template>
    <div v-if="product"
        class="fixed inset-0 bg-black/60 backdrop-blur-md z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-labelledby="quick-view-title">
        ...
        <div
            ...
            <button type="button" aria-label="Close quick view" @click="close" class="absolute top-6 right-6 z-20 w-10 h-10 bg-white/10 hover:bg-white/20 backdrop-blur-md rounded-full flex items-center justify-center transition-all">
            ...
            <div class="w-full md:w-1/2 p-8 md:p-12 flex flex-col justify-center space-y-6">
                <div>
                    ...
                    <h2 id="quick-view-title" class="font-[Playfair_Display] text-3xl font-bold mt-2">{{ product.name }}</h2>
