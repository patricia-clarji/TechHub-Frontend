...
const selectProduct = (product) => {
    ...
    searchQuery.value = '';
    router.push({ name: 'ProductDetail', params: { identifier: product.slug || product.id } });
};
</script>

<template>
    <div v-if="uiStore.searchModalOpen" class="fixed inset-0 z-[120] bg-black/40 backdrop-blur-md flex items-start justify-center p-6" role="dialog" aria-modal="true" aria-labelledby="search-title">
        ...
        <div class="relative w-full max-w-2xl bg-[var(--bg-card)] border border-[var(--border)] rounded-[2.5rem] p-8 shadow-2xl animate-[zoomIn_0.3s_ease]">
            <div class="flex justify-between items-center mb-6">
                <h3 id="search-title" class="font-[Playfair_Display] text-2xl font-bold">Search TechHub</h3>
                <button type="button" aria-label="Close search" @click="uiStore.toggleSearch()" class="w-10 h-10 rounded-full hover:bg-[var(--bg-muted)] transition-colors flex items-center justify-center">
            ...
            <div v-if="searchQuery && filteredProducts.length > 0" class="mt-6 space-y-3 max-h-80 overflow-y-auto custom-scrollbar">
                <button type="button" v-for="product in filteredProducts" :key="product.id"
                    ...
                    class="w-full text-left flex items-center gap-4 p-3 rounded-xl hover:bg-[var(--bg-muted)] transition-colors cursor-pointer">
                    <img :src="product.img" :alt="product.name" loading="lazy" class="w-12 h-12 object-cover rounded-lg" />
                    <div class="flex-1">
