    ...
    >
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center w-full">
            ...
            <div :class="getAnimationClass('relative hero-img-wrap')">
                <div class="relative z-10 rounded-[2.5rem] overflow-hidden shadow-2xl bg-[var(--bg-muted)] aspect-square lg:aspect-video flex items-center justify-center">
                    <video 
                        v-if="videoUrl"
                        ref="videoRef"
                        :src="videoUrl"
                    ...
                    <div class="absolute bottom-6 right-6 backdrop-blur-md bg-white/10 border border-white/20 px-6 py-3 rounded-2xl shadow-xl">
                ...
                <div class="absolute -inset-4 bg-gradient-to-tr from-[var(--accent)] to-transparent opacity-10 blur-3xl -z-0"></div>
            </div>
            ...
            <div class="space-y-10">
                <div :class="getAnimationClass('space-y-4 delay-100')">
                    <span class="section-badge inline-flex">
                        <span class="w-1.5 h-1.5 rounded-full bg-[var(--accent)] inline-block animate-pulse mr-3"></span>
                    ...
                    <div v-if="product.averageRating > 0" class="flex items-center gap-1">
                        ...
                        <span class="text-sm text-[var(--text-muted)] ml-2">({{ product.reviews.length }} reviews)</span>
                    </div>
                </div>
                ...
                <div class="space-y-4">
                    ...
                    >
                        <div class="w-10 h-10 rounded-full bg-[var(--bg-muted)] flex items-center justify-center text-[var(--accent)] group-hover:scale-110 transition-transform">
                            <i :class="['fa-solid', feature.icon]"></i>
                        </div>
                        <span class="font-bold text-sm uppercase tracking-widest text-[var(--text)]">{{ feature.text }}</span>
                    </div>
                </div>
