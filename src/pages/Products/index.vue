...
// Watch route query params and sync with store
watch(() => route.query.category, (newCat) => {
    if (newCat) {
        productsStore.filters.category = newCat;
    } else if (!route.query.brand) {
        // Only reset if no other filters are active
        productsStore.filters.category = 'All';
    }
}, { immediate: true });
...
watch(() => productsStore.filters.category, (newCategory) => {
    const currentCategory = route.query.category;
    if (newCategory !== 'All' && newCategory !== currentCategory) {
        router.push({ query: { ...route.query, category: newCategory } });
    } else if (newCategory === 'All' && currentCategory) {
        const { category, ...rest } = route.query;
        router.push({ query: rest });
    }
});

watch(() => productsStore.filters.brands, (newBrands) => {
    const currentBrand = route.query.brand;
    if (newBrands.length > 0 && newBrands[0] !== currentBrand) {
        router.push({ query: { ...route.query, brand: newBrands[0] } });
    } else if (newBrands.length === 0 && currentBrand) {
        ...
        <div class="flex flex-col lg:flex-row gap-12">
            ...
            <div class="flex-1">
                ...
                <div v-if="productsStore.filteredProducts.length > 0">
                    <div v-if="isLoading || productsStore.loading"
                ...
                <div v-else
                    ...
                    <p class="text-[var(--text-muted)] text-sm mt-2">Adjust your filtering parameters to expand the
                        search matrix.</p>
                    <button @click="productsStore.resetFilters"
